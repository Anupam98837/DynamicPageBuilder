...
<div class="cm-wrap">
  ...
  <div class="tab-content mb-3">
    ...
    <div class="tab-pane fade show active" id="tab-active" role="tabpanel">
      ...
      {{-- Accordion instead of flat table --}}
      ...
      <div class="modal-body">
        ...
        <div class="row g-3">
          ...
          <div class="col-md-6">
            ...
            <select id="pv_action_select" class="form-select">
            ...
            <input id="pv_action_other" class="form-control mt-2 d-none" maxlength="60" placeholder="Custom action, e.g. approve_enquiry">
          </div>
...
(function(){
  ...
  function renderActiveAccordion(items, pagination){
    ...
    const groups = {};
    items.forEach(r=>{
      const key = String(r.module_id || r.module_uuid || r.module || r.module_name || '0');
      if(!groups[key]){
        groups[key] = {
          module_id: r.module_id || null,
          module_uuid: r.module_uuid || null,
    ...
    Object.values(groups).forEach(g=>{
      ...
      // Build dropdown: remove default actions already present (per module)
  ...
  async function loadModulesInto(selectEl){
    if(!selectEl) return;
    try{
      const res = await fetch('/api/modules?per_page=500',{
        headers:{
          'Authorization':'Bearer '+TOKEN,
          'Accept':'application/json'
        }
  ...
  async function openEditPriv(key){
    try{
      ...
      pv.mode.value='edit';
      pv.key.value=(r.uuid || r.id);
      pv.title.textContent='Edit Privilege';
      ...
      if(DEFAULT_ACTIONS.includes(act)){
        pv.actionSelect.value = act;
        pv.actionOther.value = '';
        pv.actionOther.classList.add('d-none');
      }else{
        pv.actionSelect.value = '__other';
      ...
      err(e.message||'Failed to open');
    }
  }

  // Handle action dropdown (modal)
  pv.actionSelect.addEventListener('change', ()=>{
    if(pv.actionSelect.value === '__other'){
      pv.actionOther.classList.remove('d-none');
      pv.actionOther.focus();
    }else{
      pv.actionOther.classList.add('d-none');
      pv.actionOther.value='';
    }
  ...
  pv.save && pv.save.addEventListener('click', async ()=>{
    ...
    let finalAction = '';
    const selVal = pv.actionSelect.value;
    if(!selVal){
      return Swal.fire('Action required','Please select an action.','info');
    }
    if(selVal === '__other'){
      const txt = (pv.actionOther.value || '').trim();
      if(!txt) return Swal.fire('Custom action','Please type custom action.','info');
      finalAction = txt;
    }else{
      finalAction = selVal;
    }
  ...
  async function loadBulkPrivilegesForModule(moduleVal){
    ...
    try{
      const res = await fetch(`/api/privileges?module_id=${encodeURIComponent(moduleVal)}&per_page=500`,{
        headers:{'Authorization':'Bearer '+TOKEN,'Accept':'application/json'}
      });
      const j=await res.json().catch(()=>({}));
      const items = j.data || [];
